...
import { batchDelete, batchParse, batchRetry, deleteDocument, getContent, getMarkdown, getParseStatus, getSourceUrl, listDocuments, listParseTasks, listVersions, parseDocument, retryParse, uploadDocument } from '../api/documents'
...
export function DocumentsPage() {
  ...
  const refreshDocuments = async (nextPage = page, nextPageSize = pageSize) => {
    setLoadingList(true)
    try {
      const result = await listDocuments({ page: nextPage, pageSize: nextPageSize, keyword: keyword.trim() || undefined, status: statusFilter })
  ...
  const loadStatus = async (item: Document, silent = false) => {
    try {
      ...
      const nextTasks = await listParseTasks(item.id); setTasks(nextTasks)
  ...
  const selectDocument = async (item: Document) => {
    setDocument(item); setTask(undefined); setTasks([]); setMarkdown(''); setContent(undefined); setVersions([]); setSelectedVersion(undefined)
  ...
  useEffect(() => { void refreshDocuments(1) }, [])
  useEffect(() => {
  ...
  const action: UploadProps['beforeUpload'] = async file => {
    try { await uploadDocument(file); await refreshDocuments(1); message.success(`${file.name} 已上传，可开始解析`) } catch (error) { message.error(String(error)) }
    return false
  }
  ...
  return <Layout style={{ minHeight: '100vh', background: '#f5f7fa' }}><Layout.Content style={{ maxWidth: 1180, width: '100%', margin: '40px auto', padding: '0 20px' }}>
    ...
    <Card title="上传文档"><Upload multiple accept=".pdf,.png,.jpg,.jpeg,.webp" beforeUpload={action} showUploadList={false}><Button type="primary">选择文件并上传</Button></Upload><Typography.Text type="secondary" style={{ marginLeft: 12 }}>支持 PDF、PNG、JPG/JPEG、WEBP，单文件不超过 50 MB。</Typography.Text></Card>
    <Card title="文档列表" style={{ marginTop: 20 }} extra={<Space><Input.Search allowClear placeholder="按文件名筛选" style={{ width: 220 }} value={keyword} onChange={event => setKeyword(event.target.value)} onSearch={() => void refreshDocuments(1)} /><Select allowClear placeholder="全部状态" style={{ width: 130 }} value={statusFilter} onChange={value => { setStatusFilter(value); window.setTimeout(() => void refreshDocuments(1), 0) }} options={['UPLOADED', 'QUEUED', 'PARSING', 'PARSED', 'FAILED'].map(value => ({ value, label: value }))} /></Space>}>
